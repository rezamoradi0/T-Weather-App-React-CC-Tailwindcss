    ...
    this.apiLimit="3";
       this.SearchCity=(_cityName)=>{
        ...
        this.setState({cityResult:[]},()=>{
            fetch(this.apiCity).then(result=>result.json()).then(jsonRes=>{
        ...
        this.SearchWeather=(_cityObj)=>{
            ...
            fetch(this.apiWeather).then(result=>result.json()).then(jsonResult=>{
    ...
    render() { 
        return (<div className='w-11/12 h-9/10 border-8 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg border-gray-200 border-opacity-20 overflow-hidden' >
              <div className='absolute z-10 w-full h-full flex'>
                <Information IconDic={this.IconsDictaionry} Weather={this.state.selectedCityWeather}>
                <Menu IconDic={this.IconsDictaionry} Weather={this.state.selectedCityWeather} SearchCity={ this.SearchCity} SelectedCitys={this.state.cityResult} SelectCity={this.SelectCity} />
                </Information>
                <Menu IconDic={this.IconsDictaionry} Weather={this.state.selectedCityWeather} SearchCity={ this.SearchCity} SelectedCitys={this.state.cityResult} SelectCity={this.SelectCity} />
            </div>
            <video key={this.props.VideoLink} width="1280" height="1024" loop autoPlay muted className='absolute object-cover object-center w-full h-full scale-115'>
